...
returns struct<num_video_nodes int64, video_nodes_attributes array<string>>
language js
as
    '''
var result = {};
...
select
    ...
    ) as page_with_video_nodes_pct,
    safe_divide(
        countif('preload' in unnest(media_info.video_nodes_attributes)),
        countif(media_info.num_video_nodes > 0)
    ) as page_with_video_preload_pct
from
    (
        select
