...
create temp function is_ni(
    ...
    good / (good + needs_improvement + poor) < 0.75 and poor / (
...
create temp function is_poor(
    good float64, needs_improvement float64, poor float64
) returns bool as (poor / (good + needs_improvement + poor) >= 0.25)
;
...
with
    base as (
        ...
        where device in ('desktop', 'phone') and date in ('2019-08-01', '2020-08-01')
    )
