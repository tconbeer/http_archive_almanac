...
select
    ...
    sum(if(script like '%async%', 1, 0))
    / sum(if(script like '%src%', 1, 0)) as pct_external_async,
    sum(if(script like '%defer%', 1, 0))
    / sum(if(script like '%src%', 1, 0)) as pct_external_defer,
    sum(if(script like '%module%', 1, 0))
    / sum(if(script like '%src%', 1, 0)) as pct_external_module,
    sum(if(script like '%nomodule%', 1, 0))
    / sum(if(script like '%src%', 1, 0)) as pct_external_nomodule
from
    (
        select
            client,
