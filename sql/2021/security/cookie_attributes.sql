...
select
    ...
    countif(regexp_contains(cookie_value, r'(?i);.*httponly')) / count(
        0
    ) as pct_httponly,
    ...
    countif(regexp_contains(cookie_value, r'(?i);.*samesite\s*=')) / count(
        0
    ) as pct_samesite,
    countif(
        regexp_contains(cookie_value, r'(?i);.*samesite\s*=\s*lax')
    ) as count_samesite_lax,
    countif(regexp_contains(cookie_value, r'(?i);.*samesite\s*=\s*lax')) / count(
        0
    ) as pct_samesite_lax,
    countif(
        regexp_contains(cookie_value, r'(?i);.*samesite\s*=\s*strict')
    ) as count_samesite_strict,
    countif(regexp_contains(cookie_value, r'(?i);.*samesite\s*=\s*strict')) / count(
        0
    ) as pct_samesite_strict,
    countif(
        regexp_contains(cookie_value, r'(?i);.*samesite\s*=\s*none')
    ) as count_samesite_none,
    countif(regexp_contains(cookie_value, r'(?i);.*samesite\s*=\s*none')) / count(
        0
    ) as pct_samesite_none,
    countif(regexp_contains(cookie_value, r'(?i);.*sameparty')) as count_sameparty,
    countif(regexp_contains(cookie_value, r'(?i);.*sameparty')) / count(
        0
    ) as pct_sameparty,
    countif(regexp_contains(cookie_value, r'(?i);.*max-age\s*=\s*.+')) as count_max_age,
    countif(regexp_contains(cookie_value, r'(?i);.*max-age\s*=\s*.+')) / count(
        0
    ) as pct_max_age,
    countif(regexp_contains(cookie_value, r'(?i);.*expires\s*=\s*.+')) as count_expires,
    countif(regexp_contains(cookie_value, r'(?i);.*expires\s*=\s*.+')) / count(
        0
    ) as pct_expires,
    countif(
        not (
            regexp_contains(
                cookie_value, r'(?i);.*max-age\s*=\s*.+'
    ...
    countif(regexp_contains(cookie_value, r'(?i)^\s*__Secure-')) / count(
        0
    ) as pct_secure_prefix,
    countif(regexp_contains(cookie_value, r'(?i)^\s*__Host-')) as count_host_prefix,
    countif(regexp_contains(cookie_value, r'(?i)^\s*__Host-')) / count(
        0
    ) as pct_host_prefix
from
    (
