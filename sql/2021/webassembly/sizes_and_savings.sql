select
    ...
    approx_quantiles(
        size.total_strip, 1000) [offset (percentile * 10)
    ] as size_total_strip,
    approx_quantiles(
        size.total_strip_br, 1000) [offset (percentile * 10)
    ] as size_total_strip_br,
    approx_quantiles(size.total_opt, 1000) [offset (percentile * 10)] as size_total_opt,
    approx_quantiles(
        size.total_opt_br, 1000) [offset (percentile * 10)
    ] as size_total_opt_br,
    approx_quantiles(
        (raw_size - size.total_br), 1000) [offset (percentile * 10)
    ] as br_savings,
    approx_quantiles(
        (size.total_br - size.total_strip_br), 1000) [offset (percentile * 10)
    ] as strip_br_savings,
    approx_quantiles(
        (size.total_strip_br - size.total_opt_br), 1000) [offset (percentile * 10)
    ] as opt_br_savings,
    approx_quantiles(
        (size.total_strip - size.total_opt), 1000) [offset (percentile * 10)
    ] as opt_savings
from
