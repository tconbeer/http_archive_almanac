...
try {
  ...
  return Object.entries(calc.number_of_operators).map(([num, freq]) => ({num, freq}))
} catch (e) {
  return [];
}
'''
...
from
    (
        select client, url, operators.num, operators.freq
        from
            `httparchive.almanac.parsed_css`,
            unnest(getcalcoperatorcomplexity(css)) as operators
        where
            date = '2021-07-01'
            and
