...
select
    ...
    countif(not uses_cache_control and not uses_expires) / countif(
        not uses_no_store
    ) as pct_using_neither,
    countif(not uses_no_store and uses_max_age and exp_age = 0) / countif(
        not uses_no_store
    ) as pct_using_exp_age_zero,
    countif(not uses_no_store and uses_max_age and exp_age > 0) / countif(
        not uses_no_store
    ) as pct_using_exp_age_gt_zero,
